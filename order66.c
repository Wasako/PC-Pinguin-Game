...
int main(int argc, char** argv) {
    ...
    PlaceTime = ppenguins - pengs[playerID];
	if (PlaceTime)
		Place(input);
	else
		Move(input);
	...
	//Exit(argv[6+row*column+players]);
...
void InpPoints(){
	...
	for (i = 1; i <= players; i++) {
		amount = atoi(argv[5+row*column+i])
		SetPoints(i, amount);
	}
}
*/
...
void InpInput(int NotBot, int PlaceTime, int* input) {
	...
	while (1) {
		...
		damnit = (NotBot == 0) ? PlayerInp(PlaceTime, input) : Randomize(PlaceTime, input);
